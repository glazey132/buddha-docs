...
class DocumentContainer extends React.Component {
  constructor(props) {
    ...
    axios.get('http://localhost:3000/editDocument/' + this.state.id)
    .then((response) => {
      console.log('got a document payload on page load. response: ', response.data);
      this.setState({
        title: response.data.doc.title,
        editorState: response.data.doc.editorRaw ? EditorState.createWithContent(convertFromRaw(JSON.parse(response.data.editorRaw))) : this.state.editorState,
        currentSelection: response.data.doc.editorRaw ? EditorState.createWithContent(convertFromRaw(JSON.parse(response.data.editorRaw))).getSelection() : this.state.currentSelection,
        isLoading: false
      });
      console.log('the state after pageload payload was applied: ', this.state);
    })
    .catch(function (error) {
      console.log('There was an error while trying to retrieve the document on page load ', error);
    });
...
class DocumentEditor extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.socket.on('aftercolor', (obj) => {
      ...
      this.props.setStateFunction(EditorState.forceSelection(this.props.editorState, selectionState));
      const coords = window.getSelection().getRangeAt(0).getBoundingClientRect();
      this.props.setStateFunction(EditorState.forceSelection(this.props.editorState, originalSelection));
      if (obj.isCollapsed){
        this.setState({
          location: {
            top: coords.top,
    ...
    return (
      <div className='page-container'>
        <div className="document-header">
          ...
          <h6 className='document-id'>Share this ID to Collab: {this.props.docId}</h6>
        </div>
        <AppBar
